...
class HDF5Dataset(Dataset):
    ...
    def _add_data_infos(self, file_path, i):
        with h5py.File(file_path, "r") as h5_file:
            ...
            for patch_idx in range(h5_file['coords'].len()):
                ...
                self.data_info.append({'file_path': file_path, 'wsi_path': self._find_wsi(file_path), 'coords': coord, 'patch_level': patch_level, 'patch_size': patch_size})
    ...
    def _find_wsi(self, file_path):
        ...
        if len(wsi_match) != 1:
            raise Exception("could not match wsi to file_name")
